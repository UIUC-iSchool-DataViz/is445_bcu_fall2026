 ...
 * What are some examples/use cases for different kinds of colormaps or other perception channels?
 * What plot should I use when?
 * How to convert raster to vector (and visa versa)

notes:

For example of colormaps
  * excuse to look at the optional reading from week 3, in particular the FDV has some example use cases.
  * also, check out the examples and links at the bottom of the Perception in Visualization -- the examples walk through some concrete use cases and the links at the bottom are articles you can check out for more details about specific perception channels

What plot should I use when
  * this is coming, don't worry! We get to this in week 8
  * but if you want to look ahead, there are some "visualizing X" in the reading that go through more details of when to use histograms

More on raster vs vector
  * see chapter 27 of FDV for more info
  * but basically, converting from vector to raster is relatively easy you just "render" the vector to a canvas with a certain set of pixels
  * raster to vector is harder -- technically you can convert a PDF to a jpeg, but it won't contain any "instructions" for how to draw, just RGB values at a pixel level.  There are programs that try to do this though, for example for text see OCR engines

...
1. More about widgets
  * How many can we connect at once? 
  * How can we use them to make interactive plots?
2. Parts of coding that went by a bit fast/want to review what we went over in class again

notes:
For more about widgets, you're in luck!  You'll get more practice in Homework 3, but we will also be using widgets and widget-like objects today when building some dashboards with our next plotting engine bqplot (and more when we get to online platforms and vega-lite later in class)

For "want to review materials" -- go over where the videos are on Canvas, also can always preview prep notebooks
...
Still working on implementation of scaling in Canvas so the points work out -- stay tuned!
